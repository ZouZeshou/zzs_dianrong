 ...
 * @retval  
 */
void kalman1_init(kalman1_state *state, float init_x, float init_p, int16_t init_q, int16_t init_r)
{
    state->x = init_x;
    state->p = init_p;
    state->A = 1;
    state->H = 1;
    state->q = init_q;//10e-6;  /* predict noise convariance */
    state->r = init_r;//10e-5;  /* measure error convariance */  // 155 300
}
